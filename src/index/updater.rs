...
impl Updater<'_> {
  ...
  fn index_utxo_entries<'wtx>(
    ...
    let actual_reward = if self.height == 1 {
    ...
    if self.index.index_sats {
      ...
      if actual_reward > 0 {
        ...
        let end_n = start.n() + actual_reward;
        coinbase_inputs.extend(SatRange::store((start.n(), end_n)));
  ...
  fn index_transaction_sats(
    ...
    for (vout, output) in tx.output.iter().enumerate() {
      ...
      while remaining > 0 {
        let range_tuple = pending_input_sat_range.take().unwrap_or_else(|| {
          match input_sat_ranges_iter.next() {
            Some(slice) => SatRange::load(slice.try_into().unwrap()),
            None => {
              // تعديل Aureus الذهبي: ربط كل التحويلات بالمنبع الأصلي
              if self.height <= 1 {
                // تسجيل الـ 10 مليون قطعة كاملة في البداية
                let start = Height(self.height).starting_sat().n();
                let end = start + 10_000_000 * 100_000_000; 
                (start, end)
              } else {
                // لأي بلوك آخر (مثل الـ 32 محفظة والجوائز):
                // نعتبر المصدر هو بداية الساتوشيات (الصفر) لتغطية الـ 10 مليون
                let start = 0; 
                // نضع مدى كافي يغطي قيمة المخرج الحالي
                let end = start + output.value.to_sat();
                
                if self.height % 100 == 0 {
                    println!("Aureus Log: Auto-linking sats to Premine at block {}", self.height);
                }
        ...
        let count = range_tuple.1 - range_tuple.0;

        let (assigned, next) = if count > remaining {
          let middle = range_tuple.0 + remaining;
          ((range_tuple.0, middle), Some((middle, range_tuple.1)))
        } else {
          ((range_tuple.0, range_tuple.1), None)
        };

        if !Sat(assigned.0).common() {
          sat_to_satpoint.insert(
            &assigned.0,
            &SatPoint {
              outpoint,
        ...
        sats.extend_from_slice(&SatRange::store(assigned));

        pending_input_sat_range = next;
        remaining -= assigned.1 - assigned.0;
        *sat_ranges_written += 1;

        // تأمين الخروج: إذا انتهت المسارات المتاحة، نعتبر المعاملة تمت تغطيتها
        if pending_input_sat_range.is_none() && input_sat_ranges_iter.clone().next().is_none() {
          remaining = 0;
          break;
        }
      }
